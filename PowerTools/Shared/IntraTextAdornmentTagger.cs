...
{
    abstract class IntraTextAdornmentTagger<TTag, TAdorner> : ITagger<IntraTextAdornmentTag>
        where TAdorner : UIElement
    {
        protected readonly IWpfTextView view;
        private Dictionary<SnapshotSpan, TAdorner> adornmentCache = new Dictionary<SnapshotSpan, TAdorner>();
        ...
        protected abstract TAdorner CreateAdornment(TTag data, SnapshotSpan span);


        /// <returns>True if the adornment was updated and should be kept. False to have the adornment removed from the view.</returns> 
        protected abstract bool UpdateAdornment(TAdorner adornment, TTag data);
        ...
        /// </returns> 
        protected abstract IEnumerable<Tuple<SnapshotSpan, PositionAffinity?, TTag>> GetAdornmentData(NormalizedSnapshotSpanCollection spans);
        ...
        private void AsyncUpdate()
        {
            ...
            if (snapshot != view.TextBuffer.CurrentSnapshot)
            {
                ...
                Dictionary<SnapshotSpan, TAdorner> translatedAdornmentCache = new Dictionary<SnapshotSpan, TAdorner>();
        ...
        private IEnumerable<TagSpan<IntraTextAdornmentTag>> GetAdornmentTagsOnSnapshot(NormalizedSnapshotSpanCollection spans)
        {
            ...
            foreach (var spanDataPair in GetAdornmentData(spans).Distinct(new Comparer()))
            {
                ...
                TTag adornmentData = spanDataPair.Item3;
        ...
        private class Comparer : IEqualityComparer<Tuple<SnapshotSpan, PositionAffinity?, TTag>>
        {
            public bool Equals(Tuple<SnapshotSpan, PositionAffinity?, TTag> x, Tuple<SnapshotSpan, PositionAffinity?, TTag> y)
            {
                if (x == null && y == null)
                    return true;
                if (x == null || y == null)
            ...
            public int GetHashCode(Tuple<SnapshotSpan, PositionAffinity?, TTag> obj)
            {
