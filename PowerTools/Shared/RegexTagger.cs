...
namespace VarIdentifier.Shared
{
    ...
    internal abstract class RegexTagger<T> : ITagger<T> where T : ITrackingTag
    {
        private readonly IEnumerable<Regex> matchExpressions;

        public ITextBuffer Buffer { get; set; }

        public RegexTagger(ITextBuffer buffer, params Regex[] matchExpressions)
        {
        ...
        public virtual IEnumerable<ITagSpan<T>> GetTags(NormalizedSnapshotSpanCollection spans)
        {
            ...
            foreach (var line in GetIntersectingLines(spans))
            {
                ...
                foreach (var regex in matchExpressions)
                {
                    ...
                    {
                        foreach (var tag in TryCreateTagForMatch(match))
                        {
                            SnapshotSpan span = new SnapshotSpan(line.Start + tag.Item2, tag.Item3);
                            tag.Item1.Line = line;
                            yield return new TagSpan<T>(span, tag.Item1);
                        }
                    }
                }
            }
        ...
        protected abstract IEnumerable<Tuple<T, int, int>> TryCreateTagForMatch(Match match);

        protected Tuple<T, int, int> CreateMatch(T t, Group group)
        {
            return Tuple.Create(t, group.Index, group.Length);
        }
